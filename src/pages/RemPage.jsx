...
import { fetchHospitalData, monthLabel, sum } from "../lib/data";
...
export function RemPage() {
  ...
  const source = filtered.length > 0 ? filtered : nivel?.egresos || [];
  const kpis = useMemo(() => {
    ...
    const numeroEgresos = altas + fallecidos;
    return {
  ...
  if (!data || !nivel || !kpis) return <AppShell title="Gestion REM - Nivel de Cuidado">Cargando...</AppShell>;

  const years = [...new Set(data.niveles.flatMap((item) => item.egresos.map((egreso) => egreso.mes.split("-")[0])))];
  const months = [...new Set(data.niveles.flatMap((item) => item.egresos.map((egreso) => egreso.mes.split("-")[1])))].sort();
  ...
  return (
    <AppShell title="Gestion REM - Nivel de Cuidado" actions={<div className="filters"><select value={anio} onChange={(e) => setAnio(e.target.value)}><option value="">Todos</option>{years.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={mes} onChange={(e) => setMes(e.target.value)}><option value="">Todos</option>{months.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={nivelCodigo} onChange={(e) => setNivelCodigo(e.target.value)}>{data.niveles.map((item) => <option key={item.codigo} value={item.codigo}>{item.nombre}</option>)}</select><select value={glosa} onChange={(e) => setGlosa(e.target.value)}><option value="">Todas las glosas</option>{data.glosas_base.map((item) => <option key={item.clave} value={item.clave}>{item.titulo}</option>)}</select></div>}>
      <section className="kpis">
        <div className="kpi kpi-disponibles"><h4>Dias Disponibles</h4><strong>{Math.round(kpis.dias_cama_disponibles).toLocaleString("es-CL")}</strong></div>
        <div className="kpi kpi-ocupados"><h4>Dias Ocupados</h4><strong>{Math.round(kpis.dias_cama_ocupados).toLocaleString("es-CL")}</strong></div>
        <div className="kpi kpi-estada"><h4>Dias Estada</h4><strong>{Math.round(kpis.dias_estada).toLocaleString("es-CL")}</strong></div>
        <div className="kpi kpi-indice"><h4>Indice Ocupacional</h4><strong>{kpis.indice_ocupacional.toFixed(1)}%</strong></div>
      </section>

      <section className="dashboard-layout">
        <div className="main-content">
          <section className="card-glosa-dinamica"><h3 id="tituloGlosa">{currentGlosa?.titulo || "Selecciona una glosa"}</h3><div id="valorGlosa">{currentGlosa ? Math.round(kpis[currentGlosa.clave] || 0).toLocaleString("es-CL") : "--"}</div><p id="descGlosa">{currentGlosa?.descripcion || "Aqui veras el detalle del indicador"}</p></section>
          <section className="chart-full"><h3>Numero de Egresos</h3><div className="chart-container"><ChartCanvas type="bar" data={{ labels: source.map((item) => monthLabel(item.mes)), datasets: [{ label: "Altas", data: source.map((item) => item.altas), backgroundColor: "#3b82f6" }, { label: "Fallecidos", data: source.map((item) => item.fallecidos), backgroundColor: "#ef4444" }] }} options={{ responsive: true, maintainAspectRatio: false }} /></div></section>
          <section className="chart-full"><h3>Distribucion de Ocupacion</h3><div className="chart-donut-container"><ChartCanvas type="doughnut" data={{ labels: ["Ocupado", "Libre"], datasets: [{ data: [kpis.dias_cama_ocupados, Math.max(0, kpis.dias_cama_disponibles - kpis.dias_cama_ocupados)], backgroundColor: ["#3b82f6", "#f43f5e"] }] }} options={{ responsive: true, maintainAspectRatio: false, cutout: "65%" }} /></div></section>
          <section className="chart-full"><h3>Evolucion Indice Ocupacional</h3><div className="chart-container"><ChartCanvas type="line" data={{ labels: source.map((item) => monthLabel(item.mes)), datasets: [{ label: "Indice Ocupacional %", data: source.map((item) => item.dias_cama_disponibles ? Math.round((item.dias_cama_ocupados / item.dias_cama_disponibles) * 100) : 0), borderColor: "#10b981", backgroundColor: "rgba(16,185,129,0.2)", tension: 0.4, fill: true }] }} options={{ responsive: true, maintainAspectRatio: false }} /></div></section>
        </div>
